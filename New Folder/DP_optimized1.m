...
for i = N-1:-1:1            %逆序
    j = 1:ns;            %
     lb = max([(((SOC_max-SOC_grid)*Q_batt*U_oc)/-ts);Pb_min*ones(1,ns);(P_dem(i)-Pe_max)*ones(1,ns)]);          %允许的充电功率，功率下限
     ub = min([(((SOC_min-SOC_grid)*Q_batt*U_oc)/-ts);Pb_max*ones(1,ns);P_dem(i)*ones(1,ns)]);                 %允许的放电功率，功率上限
    for n=1:ns
     P_batt_grid(n,:) = linspace(lb(n),ub(n),250);    
    end                                          %解决离散化问题
     ...
     SOC_next_stage = SOC_grid - (ts .* P_batt_grid' ./ (Q_batt*U_oc)); %计算每个电池功率下，当下阶段i当下SOCj的后一阶段i+1处的SOC值
